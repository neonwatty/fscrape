...
export class CachedAnalyticsService {
            ...
            if (this.options.cacheErrors) {
              this.cache.set(cacheKey, error, {
                ttl:
                  ...
                  this.getMethodTTL("getPlatformStats"),
  ...
  private async getTimeSeriesDataInternal(options: {
    ...
    const metricData =
      stats[
        `daily${options.metric.charAt(0).toUpperCase() + options.metric.slice(1)}`
      ] ?? stats.dailyRates;
  ...
  private getMethodTTL(methodName: string): number {
    ...
    return typeof this.options.ttl === "number"
      ? this.options.ttl
      : 5 * 60 * 1000;
  }
  ...
  getTimeSeriesData(
    ...
    const result = this.analytics.getTimeSeriesData(
      platform,
      startDate,
      endDate,
    );
    ...
    if (result instanceof Promise) {
      const promise = result.then((data) => {
        ...
        if (this.options.shouldCache) {
          shouldCacheResult = this.options.shouldCache(
            "getTrendingPosts",
            [limit],
            data,
          );
    ...
    this.backgroundRefreshTimer = setInterval(() => {
      ...
      this.accessedKeys.forEach((key) => {
        ...
        if (key.startsWith("getPlatformStats:")) {
          ...
          const platform =
            parts[1] === "all" ? undefined : (parts[1] as Platform);
