...
export class PerformanceProfiler extends EventEmitter {
  ...
  private handleMeasureEntry(entry: any): void {
    if (entry.duration > this.options.thresholdMs && this.options.autoLog) {
      this.logger.warn(
        `Slow operation detected: ${entry.name} (${entry.duration.toFixed(2)}ms)`,
      );
    }
  ...
  measure(
    ...
    try {
      ...
      const endTime =
        this.marks.get(endMarkName)?.timestamp || performance.now();
      ...
      if (this.options.autoLog && duration > this.options.thresholdMs) {
        this.logger.info(
          `Performance measure: ${name} took ${duration.toFixed(2)}ms`,
        );
      }
