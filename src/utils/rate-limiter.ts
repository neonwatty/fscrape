...
export class AdvancedRateLimiter implements RateLimiter {
  ...
  constructor(platform: Platform, customConfig?: Partial<RateLimitConfig>) {
    ...
    this.config = {
      ...
      initialBackoffMs:
        customConfig?.initialBackoffMs ??
        defaultConfig.initialBackoffMs ??
        1000,
      maxBackoffMs:
        customConfig?.maxBackoffMs ?? defaultConfig.maxBackoffMs ?? 60000,
      backoffMultiplier:
        customConfig?.backoffMultiplier ?? defaultConfig.backoffMultiplier ?? 2,
    };
    ...
    const result = await this.queue.add(async () => {
      ...
      try {
        ...
        if (this.config.useBackoff && this.currentBackoffMs > 0) {
          ...
          this.backoffUntil = 0;
          logger.debug(
            `${this.platform}: Backoff reset after successful request`,
          );
        }
        return result;
      } catch (error) {
  ...
  recordRateLimitError(retryAfterMs?: number): void {
    ...
    if (retryAfterMs) {
      ...
      logger.warn(
        `${this.platform}: Rate limited, retry after ${retryAfterMs}ms`,
      );
    } else {
      ...
      logger.warn(
        `${this.platform}: Rate limited, backing off ${this.currentBackoffMs}ms`,
      );
    }
  }
  ...
  getStatus(): RateLimitStatus {
    ...
    return {
      ...
      resetTime,
      isLimited:
        this.requests.length >= this.config.maxRequests ||
  ...
  private checkQuotaReset(): void {
    ...
    if (this.quotaUsage.monthly && now >= this.quotaUsage.monthly.resetTime) {
      ...
      this.quotaUsage.monthly.resetTime = new Date(
        now.getFullYear(),
        now.getMonth() + 1,
        1,
      );
