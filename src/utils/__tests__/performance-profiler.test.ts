...
describe("PerformanceProfiler", () => {
  ...
  describe("Marks", () => {
    it("should create performance marks", () => {
      ...
      expect(() =>
        profiler.mark("test-mark-2", { custom: "data" }),
      ).not.toThrow();
    });
  ...
  describe("Measures", () => {
    ...
    it("should handle measure errors gracefully", () => {
      const measure = profiler.measure(
        "invalid",
        "nonexistent-start",
        "nonexistent-end",
      );
  ...
  describe("Profile Method", () => {
    ...
    it("should handle errors in profiled functions", async () => {
      ...
      const metrics = profiler.getMetrics();
      const errorMeasure = metrics.measures.find(
        (m) => m.name === "error-operation-error",
      );
  ...
  describe("Timer", () => {
    it("should create and stop timer", async () => {
      ...
      const metrics = profiler.getMetrics();
      const timerMeasure = metrics.measures.find(
        (m) => m.name === "test-timer-timer",
      );
