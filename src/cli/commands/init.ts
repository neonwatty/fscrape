...
export function createInitCommand(): Command {
  const command = new Command("init")
    .description(
      "Initialize a new fscrape project with configuration and database",
    )
    ...
    .option(
      "-p, --platform <platform>",
      `Primary platform (${VALID_PLATFORMS.join(", ")})`,
    )
...
async function handleInit(directory: string, options: any): Promise<void> {
  ...
  if (!options.skipDatabase) {
    ...
    try {
      ...
      spinner.succeed(
        chalk.green(`Database initialized with ${migrationsRun} migrations`),
      );
      ...
      spinner.fail(
        chalk.red(`Database initialization failed: ${formatError(error)}`),
      );
      console.warn(
        chalk.yellow(
          formatWarning(
            "You can manually initialize the database later using 'fscrape migrate'",
          ),
        ),
      );
    }
  ...
  console.log(
    chalk.white(`  Database: ${initOptions.database || "fscrape.db"}`),
  );
  ...
  console.log(
    chalk.white("  4. Run 'fscrape --help' to see all available commands"),
  );
  ...
  if (initOptions.platform === "reddit") {
    ...
    console.log(
      chalk.white("  2. Add your client ID and secret to the config file"),
    );
  }
}
