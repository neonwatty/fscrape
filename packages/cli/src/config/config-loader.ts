...
import { validatePartialConfig, mergeConfigs, safeValidateConfig } from './config-validator.js';
...
export class ConfigLoader {
  ...
  private loadFromEnvironment(): PartialConfig | null {
    ...
    if (process.env[`${ENV_PREFIX}DRY_RUN`] === 'true') {
      config.development = { ...(config.development || ({} as any)), dryRun: true };
      hasEnvVars = true;
    }
  ...
  createCliOverrides(options: Record<string, unknown>): PartialConfig {
    ...
    // Development options
    if (options.debug) {
      overrides.development = { ...(overrides.development || ({} as any)), debug: true };
    }

    if (options.dryRun) {
      overrides.development = { ...(overrides.development || ({} as any)), dryRun: true };
    }

    // Export options
    if (options.format) {
      overrides.export = { ...(overrides.export || ({} as any)), defaultFormat: options.format };
    }

    if (options.output) {
      overrides.export = { ...(overrides.export || ({} as any)), outputDir: options.output };
    }
