...
export function createScrapeCommand(): Command {
  const command = new Command('scrape')
    ...
    .option('-p, --platform <platform>', 'Platform type (reddit, hackernews, etc.)')
    .option(
      '-l, --limit <number>',
      'Maximum number of items to scrape (Reddit: auto-paginates for limits > 100)',
      (value) => validatePositiveInt(value, 'Limit')
    )
    .option('-s, --sort-by <sort>', 'Sort order (hot, new, top, controversial, old)')
    .option('-t, --time-range <range>', 'Time range (hour, day, week, month, year, all)')
...
async function handleScrape(url: string, options: ScrapeCommandOptions): Promise<void> {
  ...
  try {
    ...
    if (options.verbose) {
      console.log(chalk.gray(formatInfo(`Loaded configuration from ${configPath}`)));
    }
  } catch (error) {
    if (options.verbose) {
      console.warn(chalk.yellow(formatWarning(`Could not load config: ${formatError(error)}`)));
    }
  }
  ...
  if (!platform) {
    throw new Error('Could not detect platform. Please specify with --platform option.');
  }
...
function displaySummary(result: ScrapeResult, verbose: boolean): void {
  ...
  if (result.metadata.totalComments) {
    console.log(chalk.cyan('Comments scraped:') + ` ${result.metadata.totalComments}`);
  }
  ...
  console.log(chalk.cyan('Scraped at:') + ` ${result.metadata.scrapedAt.toLocaleString()}`);
  ...
  if (verbose && result.posts.length > 0) {
    ...
    result.posts.slice(0, 5).forEach((post, index) => {
      ...
      if (post.score !== undefined) {
        console.log(chalk.gray(`   Score: ${post.score} | Comments: ${post.commentCount || 0}`));
      }
    });
  }
}
