...
export class BatchProcessor {
  ...
  static async loadFromFile(filePath: string): Promise<BatchConfig> {
    ...
    if (filePath.endsWith('.json')) {
      ...
      throw new Error(`Unsupported batch file format: ${path.extname(filePath)}`);
    }
  }
   ...
   */
  private static async promptForOperation(type: string): Promise<BatchOperation | null> {
    switch (type) {
      case 'scrape': {
  ...
  async execute(): Promise<BatchResult[]> {
    ...
    this.formatter.startBatch(this.config.operations.length, 'Executing operations');
  ...
  private async executeOperation(operation: BatchOperation): Promise<BatchResult> {
    ...
    const operationName = `${operation.type} ${operation.platform || ''}`.trim();
  ...
  private async executeScrape(operation: BatchOperation): Promise<any> {
    ...
    try {
      ...
      if (operation.platform === 'hackernews' || operation.platform === 'both') {
  ...
  private displayResults(): void {
    ...
    this.results.forEach((result, index) => {
      ...
      const operationName = `${result.operation.type} ${result.operation.platform || ''}`.trim();
      const duration = result.duration ? `(${result.duration}ms)` : '';

      console.log(`${statusSymbol} ${index + 1}. ${operationName} ${chalk.gray(duration)}`);
