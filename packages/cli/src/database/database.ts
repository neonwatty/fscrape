...
import type { ForumPost, Comment, User, Platform, ScrapeResult } from '../types/core.js';
...
export class DatabaseManager {
    ...
    const transaction = this.db.transaction((posts: ForumPost[]) => {
      for (const post of posts) {
        ...
        if (post.url && !post.url.startsWith('http://') && !post.url.startsWith('https://')) {
          throw new Error(`Invalid URL format for post ${post.id}: ${post.url}`);
        }
  ...
  createSession(params: {
    ...
    const now = Date.now();
    const _queryType =
      params.queryType || (params.subreddit ? 'subreddit' : params.query ? 'search' : null);
  ...
  updateSession(
    ...
    if (updates.totalPosts !== undefined) params.totalPosts = updates.totalPosts;
    if (updates.totalComments !== undefined) params.totalComments = updates.totalComments;
    if (updates.totalUsers !== undefined) params.totalUsers = updates.totalUsers;
    if (updates.errorMessage !== undefined) params.errorMessage = updates.errorMessage;
  ...
  recordMetric(
    ...
    const timeBucket = Math.floor(Date.now() / (5 * 60 * 1000)) * (5 * 60 * 1000);
  ...
  countOldData(options: { olderThanDays: number; platform?: Platform }): {
    ...
    const postCount = this.db
      .prepare(`SELECT COUNT(*) as count FROM posts WHERE created_at < ?${platformClause}`)
  ...
  deleteOldData(options: { olderThanDays: number; platform?: Platform }): {
    ...
    const transaction = this.db.transaction(() => {
      ...
      this.db.prepare('DELETE FROM scraping_sessions WHERE started_at < ?').run(cutoffTime);
    });
