...
import pRetry, { type Options as PRetryOptions, type FailedAttemptError } from 'p-retry';
  ...
  async execute<T>(fn: () => Promise<T>, shouldRetry?: (error: Error) => boolean): Promise<T> {
    ...
    while (true) {
      try {
        ...
        if (this.state.retriesRemaining <= 0) {
          logger.error(`Backoff: Max retries (${this.config.maxRetries}) exceeded`);
        ...
        logger.info(`Backoff: Retry attempt ${this.state.attempt}/${this.config.maxRetries}`);
      }
    }
  }
...
export class ExponentialBackoff extends BaseBackoffStrategy {
  calculateDelay(attempt: number): number {
    ...
    return this.config.initialDelayMs * Math.pow(this.config.multiplier, attempt - 1);
  }
}
...
export class BackoffFactory {
   ...
   */
  static fibonacci(initialDelayMs = 1000, maxDelayMs = 60000): FibonacciBackoff {
   ...
   */
  static decorrelated(initialDelayMs = 1000, maxDelayMs = 20000): DecorrelatedBackoff {
...
export class AdvancedRetry {
  ...
  constructor(config: AdvancedRetryConfig = {}, backoffStrategy?: BaseBackoffStrategy) {
    this.config = config;
  ...
  static withLinearBackoff(
    ...
    const backoff = BackoffFactory.linear(backoffConfig?.initialDelayMs, backoffConfig?.maxDelayMs);
    return new AdvancedRetry(config ?? {}, backoff);
  }
