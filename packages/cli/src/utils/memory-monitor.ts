...
export class MemoryMonitor {
  ...
  async analyzeMemoryLeaks(): Promise<{
    ...
    if (possibleLeak) {
      recommendation = 'Possible memory leak detected. Consider profiling the application';
  ...
  getMemoryDelta(): {
    ...
    return {
      ...
      percentChange:
        ((current.heapUsed - this.baselineMemory.heapUsed) / this.baselineMemory.heapUsed) * 100,
    };
  }
}
