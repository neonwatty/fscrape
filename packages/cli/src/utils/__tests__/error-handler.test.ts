...
describe('ErrorHandler', () => {
  ...
  describe('Retry Logic', () => {
    ...
    it('should not retry non-retryable errors', async () => {
      ...
      await expect(errorHandler.executeWithRetry(operation)).rejects.toThrow('Invalid input');
      expect(operation).toHaveBeenCalledOnce();
    });
  ...
  describe('Circuit Breaker', () => {
    it('should open circuit after failure threshold', async () => {
      const operation = vi.fn().mockRejectedValue(new NetworkError('Service unavailable'));
      ...
      await expect(handler.executeWithCircuitBreaker(operation, 'test_service')).rejects.toThrow(
  ...
  describe('User Notifications', () => {
    it('should notify on errors when enabled', async () => {
      ...
      const handler = new ErrorHandler(undefined, undefined, notificationConfig);
      const operation = vi
        .fn()
        .mockRejectedValue(new BaseError('Test error', 'TEST_ERROR', ErrorSeverity.HIGH));
    ...
    it('should use custom notifier when provided', async () => {
      ...
      const operation = vi
        .fn()
        .mockRejectedValue(new BaseError('Test error', 'TEST_ERROR', ErrorSeverity.CRITICAL));
    ...
    it('should notify recovery attempts when enabled', async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Attempting recovery'));
    });
  });
  ...
  describe('Graceful Degradation', () => {
    it('should degrade service after threshold failures', async () => {
      const degradationConfig: DegradationConfig = {
        enabled: true,
        fallbackServices: new Map([['test_service', () => 'degraded_response']]),
        degradationThreshold: 3,
        autoRecover: false,
        recoveryCheckInterval: 5000,
      };

      const handler = new ErrorHandler(undefined, undefined, undefined, degradationConfig);
  ...
  describe('Recovery Strategies', () => {
    ...
    it('should apply exponential backoff for rate limits', async () => {
      const handler = new ErrorHandler();
      const rateLimitError = new RateLimitError('Rate limited', 429, 60, 0, Date.now() + 1000);
    ...
    it('should terminate on critical errors', async () => {
      const handler = new ErrorHandler();
      const error = new BaseError('Critical failure', 'CRITICAL', ErrorSeverity.CRITICAL);
      error.recoveryStrategy = RecoveryStrategy.TERMINATE;

      const operation = vi.fn().mockRejectedValue(error);

      await expect(handler.handle(operation, { name: 'test_op' })).rejects.toThrow(
        'Critical failure'
      );
    ...
    it('should ignore errors with ignore strategy', async () => {
      const handler = new ErrorHandler();
      const error = new BaseError('Ignorable error', 'IGNORE', ErrorSeverity.LOW);
  ...
  describe('Performance', () => {
    it('should handle concurrent operations', async () => {
      ...
      const promises = operations.map((op, i) => handler.handle(op, { name: `operation_${i}` }));
    ...
    it('should isolate circuit breakers per operation', async () => {
      ...
      const result = await handler.executeWithCircuitBreaker(successOp, 'working_service');
      expect(result).toBe('success');
