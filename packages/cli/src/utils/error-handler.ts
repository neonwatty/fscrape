...
export class ErrorHandler {
  ...
  public async handle<T>(
    ...
    try {
      ...
      return this.applyRecoveryStrategy(baseError, operation, recoveryStrategy, context);
    }
  }
   ...
   */
  public async executeWithCircuitBreaker<T>(operation: () => Promise<T>, name: string): Promise<T> {
    const circuitBreaker = this.getOrCreateCircuitBreaker(name);

    if (!circuitBreaker.canExecute()) {
      throw new NetworkError(`Circuit breaker is open for: ${name}`, 'CIRCUIT_BREAKER_OPEN');
    }
