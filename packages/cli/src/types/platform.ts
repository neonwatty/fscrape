...
export const RedditConfigSchema = z.object({
  ...
  timeRange: z.enum(['hour', 'day', 'week', 'month', 'year', 'all']).default('day'),
  includeNSFW: z.boolean().default(false),
  includeStickied: z.boolean().default(true),
  expandComments: z.boolean().default(false),
  commentSort: z.enum(['best', 'top', 'new', 'controversial', 'old', 'qa']).default('best'),
});
...
export type PlatformResponseMetadata = z.infer<typeof PlatformResponseMetadataSchema>;
