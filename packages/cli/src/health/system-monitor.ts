...
export class SystemMonitor {
  ...
  private checkProcess(process: SystemMetrics['process']): HealthCheckResult {
    ...
    const message =
      heapPercent > 90 ? `Process heap usage high: ${heapPercent.toFixed(1)}%` : `Process healthy`;
  ...
  public getAverageMetrics(): Partial<SystemMetrics> {
    ...
    const avgCpu =
      this.metricsHistory.reduce((sum, m) => sum + m.cpu.usage, 0) / this.metricsHistory.length;
    const avgMemory =
