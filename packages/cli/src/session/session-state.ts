...
export class SessionStateManager {
   ...
   */
  createSession(id: string, platform: Platform, config?: SessionState['config']): SessionState {
   ...
   */
  updateProgress(id: string, update: Partial<SessionState['progress']>): void {
    const state = this.states.get(id);
    if (state) {
      ...
      if (state.progress.processedItems > 0) {
        state.metrics.averageItemTime = state.metrics.totalTime / state.progress.processedItems;
      }
    }
  }
   ...
   */
  addError(id: string, error: Error | string, itemId?: string): void {
    const state = this.states.get(id);
    if (state) {
      const errorEntry = {
        timestamp: new Date(),
   ...
   */
  updateResumeData(id: string, resumeData: SessionState['resumeData']): void {
    const state = this.states.get(id);
    if (state) {
   ...
   * Update metrics
   */
  updateMetrics(id: string, metrics: Partial<SessionState['metrics']>): void {
    const state = this.states.get(id);
    if (state) {
