...
import { SessionStateManager, type SessionState, type SessionStatus } from './session-state.js';
...
export class SessionManager extends EventEmitter {
  ...
  async createSession(config: SessionConfig): Promise<SessionState> {
    ...
    // Create new session
    const session = this.stateManager.createSession(sessionId, config.platform, {
   ...
   */
  updateProgress(sessionId: string, processedItems: number, totalItems?: number): void {
    this.stateManager.updateProgress(sessionId, {
      processedItems,
      totalItems: totalItems || this.stateManager.getSession(sessionId)?.progress.totalItems || 0,
    });

    const progress = this.progressTracker.updateProgress(sessionId, processedItems, totalItems);
   ...
   */
  updateMetrics(sessionId: string, metrics: Partial<SessionState['metrics']>): void {
   ...
   */
  updateResumeData(sessionId: string, resumeData: SessionState['resumeData']): void {
  ...
  private setupProgressEvents(): void {
    ...
    this.progressTracker.on('progress:milestone', (milestone) => {
      this.logger.info(`Session ${milestone.sessionId} reached ${milestone.milestone}% milestone`);
    });
  }
