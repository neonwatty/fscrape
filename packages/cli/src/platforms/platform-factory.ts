...
export class PlatformFactory {
   ...
   */
  static async create(platform: Platform, options: FactoryOptions = {}): Promise<BasePlatform> {
    const {
      config,
      scraperConfig,
      platformConfig,
    ...
    if (useRateLimiter) {
      const rateLimitConfig = scraperConfig?.rateLimit || this.getDefaultRateLimitConfig();
  ...
  private static buildConfiguration(
      ...
      try {
        ...
        if (managedConfig) {
          ...
          if (managedConfig.rateLimit?.maxRequestsPerMinute) {
            finalConfig.rateLimitPerMinute = managedConfig.rateLimit.maxRequestsPerMinute;
          }
        }
      } catch (error) {
    ...
    if (scraperConfig) {
      ...
      if (scraperConfig.rateLimit?.maxRequestsPerMinute) {
        finalConfig.rateLimitPerMinute = scraperConfig.rateLimit.maxRequestsPerMinute;
      }
    }
   ...
   */
  static async testPlatform(platform: Platform, config: BasePlatformConfig = {}): Promise<boolean> {
    try {
      ...
      this.logger.error(`Platform "${platform}" connection test failed:`, error);
      return false;
    }
  }
  ...
  static getPlatformRequirements(platform: Platform): {
    ...
    const requirements: Record<Platform, { required: string[]; optional: string[] }> = {
      reddit: {
