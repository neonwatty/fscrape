...
export class PlatformRegistry {
  ...
  static register(
    platform: Platform,
    constructor: PlatformConstructor,
    metadata?: Omit<Partial<PlatformMetadata>, 'name' | 'constructor' | 'capabilities'>
  ): void {
   ...
   */
  static listByCapability(capability: keyof BasePlatformCapabilities): Platform[] {
    if (!this.initialized) {
      this.initializeDefaultsSync();
    }
  ...
  private static async loadPlugin(pluginPath: string): Promise<void> {
    try {
      ...
      this.register(manifest.platform, PlatformClass as PlatformConstructor, metadata);
