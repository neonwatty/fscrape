...
export class DataFilter {
  ...
  filterPosts(posts: ForumPost[]): ForumPost[] {
    ...
    if (this.options.authors && this.options.authors.length > 0) {
      const authors = new Set(
        this.options.authors.map((a) => (this.options.caseSensitive ? a : a.toLowerCase()))
      );
    ...
    if (this.options.excludeAuthors && this.options.excludeAuthors.length > 0) {
      const excludeAuthors = new Set(
        this.options.excludeAuthors.map((a) => (this.options.caseSensitive ? a : a.toLowerCase()))
      );
      filtered = filtered.filter((p) => {
        const author = this.options.caseSensitive ? p.author : p.author.toLowerCase();
  ...
  private sortPosts(posts: ForumPost[]): ForumPost[] {
    ...
    switch (this.options.sortBy) {
      case 'date':
        sorted.sort(
          (a, b) => multiplier * (new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
        );
        break;
      case 'score':
...
export class FilterChain {
  ...
  apply(data: ScrapeResult): ScrapeResult {
    return this.filters.reduce((result, filter) => filter.filterScrapeResult(result), data);
  }

  applyToPosts(posts: ForumPost[]): ForumPost[] {
    return this.filters.reduce((result, filter) => filter.filterPosts(result), posts);
  }
}
